...
export default function Form() {
  ...
  const onSubmit = async (data: User) => {
    try {
      if (profilePicture) {
        // Se uma nova foto de perfil foi carregada durante o cadastro,
        // faça o upload da nova foto e obtenha a URL
        const profilePictureUrl = await uploadProfilePicture(
          profilePicture,
          id || "",
        );
        // Defina a URL da nova foto de perfil no objeto de dados
        data.profilePicture = profilePictureUrl || "";
      } else if (userData.profilePicture) {
        // Se não houver uma nova foto de perfil durante o cadastro, mas
        // o usuário já tem uma foto de perfil existente, mantenha a foto atual
        data.profilePicture = userData.profilePicture;
      }

      if (!id) {
        // Adicione um novo usuário
        await adicionarUsuarioAoFirestore(data);
      } else {
        ...
        setUsers(newUsers);
        await atualizarUsuarioNoFirestore(data, id);
      }
  ...
  const handleProfilePictureChange = (
    ...
    if (files && files.length > 0 && !profilePicture) {
      setProfilePicture(files[0]);
    }
  };

  const handleAvatarClick = () => {
    // Verifica se há uma imagem de perfil definida
    if (profilePicture) {
      // Se houver, você pode exibir um aviso ou simplesmente não fazer nada
      console.log("Já existe uma imagem de perfil definida.");
      return; // Não faz nada
    }
    // Se não houver uma imagem de perfil definida, clique para selecionar uma
  ...
  const formatCelular = (value: string) => {
    ...
    // Aplicar formatação
    if (value.length === 11) {
      value = `(${value.substr(0, 2)}) ${value.substr(2, 5)}-${value.substr(
        7,
      )}`;
    }
    return value;
